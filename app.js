...
function waypointFromPoint(c,props={}){if(!Array.isArray(c))return null;let a=Number(c[0]),b=Number(c[1]);const ele=c[2]==null?null:Number(c[2]);if(!Number.isFinite(a)||!Number.isFinite(b))return null;let lng=a,lat=b;if(Math.abs(a)<=90&&Math.abs(b)>90){lat=a;lng=b}if(Math.abs(lat)>90||Math.abs(lng)>180)return null;return{lat,lng,ele:Number.isFinite(ele)?ele:null,name:props.name||props.title||props.label||'Waypoint',desc:props.desc||props.description||'',type:props.type||props.category||''}}
...
function parseGeoJSON(text){
  ...
  const lines=[],boundaries=[],waypoints=[],orderedPoints=[],seenTypes=new Set();
  const isPair=c=>Array.isArray(c)&&c.length>=2&&Number.isFinite(Number(c[0]))&&Number.isFinite(Number(c[1]));
  const addLine=(c,source='line')=>{if(Array.isArray(c)&&c.length>=2&&c.every(isPair))(source==='boundary'?boundaries:lines).push(c)};
  ...
  // Be tolerant of route exports that are JSON/GeoJSON-like but omit geometry.type.
  // We only accept explicit arrays made of coordinate pairs; arbitrary numbers are ignored.
  const fallbackSeq=[];
  const scan=(v,key='')=>{
    if(!v||typeof v!=='object')return;
    if(Array.isArray(v)){
      if(v.length>=2&&v.every(isPair)){fallbackSeq.push(v);return}
      v.forEach(x=>scan(x,key));return;
    }
    for(const [k,val] of Object.entries(v)){
      if(['bbox'].includes(k))continue;
      if(['coordinates','coords','points','track','path','route','geometry'].includes(k.toLowerCase())||typeof val==='object')scan(val,k);
    }
  };
  ...
  const normalizeLine=c=>{
    ...
    // Standard GeoJSON is [lon,lat]. Some mobile/export tools incorrectly write [lat,lon].
    // Auto-detect when one axis is outside latitude range; Indonesia longitudes make this reliable.
    let reverse=false;
    for(const x of c){if(!isPair(x))continue;const a=Number(x[0]),b=Number(x[1]);if(Math.abs(a)<=90&&Math.abs(b)>90){reverse=true;break}if(Math.abs(a)>90&&Math.abs(b)<=90){reverse=false;break}}
    return c.map(x=>{const a=Number(x?.[0]),b=Number(x?.[1]),e=x?.[2]==null?null:Number(x[2]);return reverse?[a,b,Number.isFinite(e)?e:null]:[b,a,Number.isFinite(e)?e:null]}).filter(p=>Number.isFinite(p[0])&&Number.isFinite(p[1])&&Math.abs(p[0])<=90&&Math.abs(p[1])<=180);
  };

  const lineCandidates=lines.map(normalizeLine).filter(c=>c.length>=2);
  const boundaryCandidates=boundaries.map(normalizeLine).filter(c=>c.length>=2);
  let geometryKind='route',coords=[];
  if(lineCandidates.length)coords=lineCandidates.sort((a,b)=>distance(b)-distance(a))[0];
  else if(boundaryCandidates.length){coords=boundaryCandidates.sort((a,b)=>distance(b)-distance(a))[0];geometryKind='boundary'}
  if(coords.length<2){
    const types=[...seenTypes].filter(Boolean).join(', ');
    throw Error(types?`GeoJSON terbaca, tetapi tidak ada jalur/titik yang bisa digunakan. Geometry terdeteksi: ${types}.`:'GeoJSON tidak memiliki rangkaian koordinat yang dapat digunakan sebagai jalur.');
  }
  ...
  const defaultName=geometryKind==='boundary'?'Batas area GeoJSON':'Rute GeoJSON';
  return{name:props?.name||props?.title||defaultName,coords,waypoints,geometryKind};
}
