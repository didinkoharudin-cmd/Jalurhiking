...
function normalizeRouteCoords(coords=[]){
  if(!Array.isArray(coords))return{coords:[],changed:false,swapped:false};
  const rows=coords.filter(c=>Array.isArray(c)&&c.length>=2&&Number.isFinite(Number(c[0]))&&Number.isFinite(Number(c[1])));
  if(!rows.length)return{coords:[],changed:rows.length!==coords.length,swapped:false};
  let directOnly=0,swapOnly=0,directValid=0,swapValid=0,strongSwap=0,strongDirect=0;
  const step=Math.max(1,Math.floor(rows.length/250));
  for(let i=0;i<rows.length;i+=step){
    const a=Number(rows[i][0]),b=Number(rows[i][1]),direct=validLatLng(a,b),swapped=validLatLng(b,a);
    if(direct)directValid++;if(swapped)swapValid++;if(direct&&!swapped)directOnly++;if(swapped&&!direct)swapOnly++;
    if(Math.abs(a)>90&&Math.abs(a)<=180&&Math.abs(b)<=90)strongSwap++;
    if(Math.abs(b)>90&&Math.abs(b)<=180&&Math.abs(a)<=90)strongDirect++;
  }
  const swap=strongSwap>strongDirect||(swapOnly>directOnly&&swapValid>=directValid);
  const out=rows.map(c=>{const a=Number(c[0]),b=Number(c[1]),rest=c.slice(2);return swap?[b,a,...rest]:[a,b,...rest]}).filter(c=>validLatLng(c[0],c[1]));
  return{coords:out,changed:swap||out.length!==coords.length,swapped:swap};
}
function normalizeWaypoint(w){
  if(!w||typeof w!=='object')return w;let lat=Number(w.lat),lng=Number(w.lng??w.lon);
  if(!Number.isFinite(lat)||!Number.isFinite(lng))return w;
  if(!validLatLng(lat,lng)&&validLatLng(lng,lat)){const t=lat;lat=lng;lng=t;return{...w,lat,lng}}
  return w;
}
function normalizeRouteObject(route){
  if(!route||!Array.isArray(route.coords))return false;const n=normalizeRouteCoords(route.coords);let changed=n.changed;
  if(n.coords.length>=2)route.coords=n.coords;
  if(Array.isArray(route.waypoints)){const before=JSON.stringify(route.waypoints);route.waypoints=route.waypoints.map(normalizeWaypoint);if(JSON.stringify(route.waypoints)!==before)changed=true}
  if(changed)route.coordOrder='latlng';return changed;
}
function ensureMapCenter(){
  if(validLatLng(mapState.center?.[0],mapState.center?.[1]))return;
  const p=activeRoute?.coords?.find(c=>validLatLng(c?.[0],c?.[1]));mapState.center=p?[Number(p[0]),Number(p[1])]:[-2.35,117.6];
}
...
async function load(){
  ...
  let fixedLegacyRoutes=0;for(const r of routes){if(normalizeRouteObject(r)){fixedLegacyRoutes++;dbPut(STORES.routes,r).catch(()=>{})}}
  ...
  if(fixedLegacyRoutes)setTimeout(()=>toast(`${fixedLegacyRoutes} rute lama diperbaiki otomatis (urutan koordinat)`),500);
}
...
function activateRoute(r){if(!r)return;if(normalizeRouteObject(r))dbPut(STORES.routes,r).catch(()=>{});activeRoute=r;routeProximityArmed=false;localStorage.setItem('jalurnusa_active',r.id);activeTrip=tripPlans.find(t=>t.routeId===r.id)||null;const pkg=mapPackages.find(p=>p.mountainId&&p.mountainId===r.mountainId);if(pkg&&(!activeMapPackage||activeMapPackage.mountainId!==r.mountainId))activateMapPackage(pkg,false);setRoutePanelCompact(true);renderPlan();setView('mapView');requestAnimationFrame(()=>requestAnimationFrame(()=>fitActiveRoute()));toast('Rute dibuka offline')}
...
async function importRoute(file,mountainId=''){
  ...
  const normalized=normalizeRouteCoords(d.coords);if(normalized.coords.length<2)throw Error('Koordinat rute tidak valid');d.coords=normalized.coords;d.waypoints=(d.waypoints||[]).map(normalizeWaypoint);
...
function renderMap(){const st=$('#mapStage');if(!st.clientWidth)return;ensureMapCenter();updateZoomPill();updateScaleBar();renderTiles();renderRoute();updateRoutePanel()}
function updateScaleBar(){const el=$('#mapScale');if(!el)return;const lat=clamp(mapState.center[0],-85,85),mpp=156543.03392*Math.cos(lat*Math.PI/180)/(2**mapState.zoom),targetPx=74,targetM=mpp*targetPx,candidates=[1,2,5],pow=10**Math.floor(Math.log10(Math.max(targetM,1))),nice=candidates.map(x=>x*pow).filter(x=>x<=targetM).pop()||pow,px=clamp(nice/mpp,34,110);el.style.width=`${px}px`;el.textContent=nice>=1000?`${(nice/1000).toFixed(nice>=10000?0:1)} km`:`${Math.round(nice)} m`}
...
function protoLayers(source='base'){
  const textHalo={'text-halo-color':'#07110c','text-halo-width':1.5,'text-halo-blur':.35};
  return [
    {id:'background',type:'background',paint:{'background-color':'#0b1810'}},
    {id:'earth',type:'fill',source,'source-layer':'earth',paint:{'fill-color':'#18301f'}},
    {id:'landuse',type:'fill',source,'source-layer':'landuse',paint:{'fill-color':['match',['get','kind'],'forest','#1c3a25','park','#24462d','grass','#35543a','farmland','#4a4930','residential','#303a31','#233828'],'fill-opacity':['interpolate',['linear'],['zoom'],5,.78,15,.94]}},
    {id:'water',type:'fill',source,'source-layer':'water',paint:{'fill-color':'#22566a'}},
    {id:'water-outline',type:'line',source,'source-layer':'water',minzoom:8,paint:{'line-color':'#5591a6','line-width':['interpolate',['linear'],['zoom'],9,.45,18,1.25],'line-opacity':.8}},
    {id:'waterways',type:'line',source,'source-layer':'water',paint:{'line-color':'#65a5bd','line-width':['interpolate',['linear'],['zoom'],10,.75,16,1.7,22,3],'line-opacity':.95}},
    {id:'boundaries',type:'line',source,'source-layer':'boundaries',paint:{'line-color':'#89958d','line-dasharray':[3,3],'line-width':['interpolate',['linear'],['zoom'],8,.55,18,1.25],'line-opacity':.55}},
    {id:'roads-casing',type:'line',source,'source-layer':'roads',filter:['!', ['match',['get','kind'],['path','track'],true,false]],paint:{'line-color':'#07110c','line-width':['interpolate',['linear'],['zoom'],8,1.8,15,6,18,8.5,22,12]}},
    {id:'roads',type:'line',source,'source-layer':'roads',filter:['!', ['match',['get','kind'],['path','track'],true,false]],paint:{'line-color':['match',['get','kind'],'highway','#f0c56f','major_road','#d6b56f','minor_road','#b8b596','service','#909581','#a5aa92'],'line-width':['interpolate',['linear'],['zoom'],8,.65,15,2.55,18,4,22,5.8]}},
    {id:'trails-casing',type:'line',source,'source-layer':'roads',filter:['match',['get','kind'],['path','track'],true,false],paint:{'line-color':'#07110c','line-width':['interpolate',['linear'],['zoom'],11,1.8,16,3.7,20,5.9,22,7.4]}},
    {id:'trails',type:'line',source,'source-layer':'roads',filter:['match',['get','kind'],['path','track'],true,false],paint:{'line-color':'#ead89b','line-width':['interpolate',['linear'],['zoom'],11,.85,16,1.8,20,3,22,3.8],'line-dasharray':[2.2,1.25]}},
    {id:'buildings',type:'fill',source,'source-layer':'buildings',minzoom:13,paint:{'fill-color':'#647267','fill-opacity':['interpolate',['linear'],['zoom'],13,.4,18,.86]}},
    {id:'building-outline',type:'line',source,'source-layer':'buildings',minzoom:16,paint:{'line-color':'#9aa69d','line-width':['interpolate',['linear'],['zoom'],16,.35,22,1.15],'line-opacity':.7}},
    {id:'place-labels',type:'symbol',source,'source-layer':'places',minzoom:7,layout:{'text-field':['coalesce',['get','name'],['get','name_en']],'text-font':['Noto Sans Regular'],'text-size':['interpolate',['linear'],['zoom'],7,9,12,11,16,13],'text-max-width':9,'text-allow-overlap':false},paint:{'text-color':'#edf3ee',...textHalo}},
    {id:'poi-labels',type:'symbol',source,'source-layer':'pois',minzoom:13,layout:{'text-field':['coalesce',['get','name'],['get','name_en']],'text-font':['Noto Sans Regular'],'text-size':['interpolate',['linear'],['zoom'],13,9,17,11],'text-offset':[0,1.1],'text-anchor':'top','text-optional':true},paint:{'text-color':'#f1ddb0',...textHalo}}
  ];
}
...
async function renderMapLibreOffline(seq){
  ...
  if(vectorMap&&vectorMapSignature===signature){
    ...
    if(seq!==mapRenderSeq)return false;$('#mapAttrib').textContent=activeTerrainPackage?'OFFLINE TOPO':'OFFLINE MAP';$('#mapModePill').textContent=activeTerrainPackage?'TOPO + RELIEF':'OUTDOOR OFFLINE';return true;
  }
...
function renderRoute(){
  ...
  const v=viewport(),halo=$('#routeHalo'),line=$('#routeLine'),recLine=$('#recordingLine'),marks=$('#mapMarkers'),wps=$('#waypointMarkers'),mounts=$('#mountainMarkers');let pts='';marks.innerHTML='';wps.innerHTML='';if(mounts)mounts.innerHTML='';
  if(activeRoute?.coords?.length){
    pts=activeRoute.coords.map(c=>{const [x,y]=project(c[0],c[1],mapState.zoom);return `${(x-v.left).toFixed(1)},${(y-v.top).toFixed(1)}`}).join(' ');
    const a=activeRoute.coords[0],b=activeRoute.coords.at(-1),p1=project(a[0],a[1],mapState.zoom),p2=project(b[0],b[1],mapState.zoom);marks.innerHTML=markerCircle(p1[0]-v.left,p1[1]-v.top,6,'#70d49a')+markerCircle(p2[0]-v.left,p2[1]-v.top,6,'#ff8072');
    const way=(activeRoute.waypoints||[]).slice(0,100);wps.innerHTML=way.map((w,i)=>{const p=project(w.lat,w.lng,mapState.zoom),x=p[0]-v.left,y=p[1]-v.top,kind=wpKind(w),fill=({water:'#55a9ff',hazard:'#ff8072',summit:'#f5bd42',basecamp:'#70d49a',camp:'#b79cff',pos:'#f5bd42',custom:'#d6dcd8'})[kind]||'#f5bd42',name=esc(w.name||`Waypoint ${i+1}`),showName=mapState.zoom>=14;return `<g class="waypoint-group"><circle cx="${x}" cy="${y}" r="7" fill="${fill}" stroke="#07110c" stroke-width="2.4"><title>${name} • ${esc(kind)}</title></circle>${showName?`<text x="${x+10}" y="${y+4}" class="waypoint-name">${name}</text>`:`<text x="${x+9}" y="${y-8}" class="waypoint-label">${i+1}</text>`}</g>`}).join('');
  }
  if(halo)halo.setAttribute('points',pts);line.setAttribute('points',pts);
  ...
  if(mounts&&mapState.zoom>=8){const visible=mountains.filter(m=>{const p=project(m.lat,m.lng,mapState.zoom),x=p[0]-v.left,y=p[1]-v.top;return x>-90&&x<v.w+90&&y>-50&&y<v.h+50}).slice(0,40);mounts.innerHTML=visible.map(m=>{const p=project(m.lat,m.lng,mapState.zoom),x=p[0]-v.left,y=p[1]-v.top,active=activeRoute?.mountainId===m.id;return `<g class="mountain-poi ${active?'active':''}"><path d="M ${x-5} ${y+4} L ${x} ${y-5} L ${x+5} ${y+4} Z"/><text x="${x+8}" y="${y+3}">${esc(m.name)}</text></g>`}).join('')}
  renderGpsMarker();
}
...
function fitActiveRoute(){if(!activeRoute?.coords?.length)return;const valid=activeRoute.coords.filter(c=>validLatLng(c?.[0],c?.[1]));if(valid.length<2)return;const el=$('#mapStage');if(!el||el.clientWidth<120||el.clientHeight<180){requestAnimationFrame(()=>fitActiveRoute());return}const lats=valid.map(c=>Number(c[0])),lngs=valid.map(c=>Number(c[1])),minLat=Math.min(...lats),maxLat=Math.max(...lats),minLng=Math.min(...lngs),maxLng=Math.max(...lngs);mapState.center=[(minLat+maxLat)/2,(minLng+maxLng)/2];const pad=54;let chosen=2;for(let z=Math.min(20,MAX_MAP_ZOOM);z>=2;z--){const a=project(maxLat,minLng,z),b=project(minLat,maxLng,z);if(Math.abs(b[0]-a[0])<Math.max(80,el.clientWidth-pad*2)&&Math.abs(b[1]-a[1])<Math.max(120,el.clientHeight-185)){chosen=z;break}}mapState.zoom=chosen;renderMap()}
...
window.JalurNusaDiagnostics={version:'4.3-outdoor-pro',parseGPX,parseGeoJSON,detectRouteData};
